# KVM Switch Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build a software KVM switch that uses ddcutil to switch both monitors between two computers via Ctrl+Alt+P.

**Architecture:** A bash switch script reads a config file and fires parallel `ddcutil setvcp 0x60` commands. An install script handles dependency setup, interactive monitor discovery, and GNOME keybinding registration.

**Tech Stack:** Bash, ddcutil, gsettings (GNOME), i2c-dev kernel module

**Spec:** `docs/superpowers/specs/2026-03-13-kvm-switch-design.md`
...
```
kvm-at-home/
  kvm-switch.sh     # The switch script — sources config, fires ddcutil in parallel
  install.sh        # Interactive setup — installs deps, discovers monitors, writes config, registers keybinding
  README.md         # Setup and usage instructions
```

Runtime (not in repo):
```
~/.config/kvm-at-home/config   # Per-machine config with bus numbers and target input codes
```

---

## Chunk 1: The Switch Script and Install Script

> **Note:** Tasks 2-4 build up install.sh incrementally. Intermediate commits leave install.sh incomplete. This is fine for development — the script is fully functional after Task 4.

### Task 1: Create kvm-switch.sh

**Files:**
- Create: `kvm-switch.sh`

- [ ] **Step 1: Write kvm-switch.sh**
...
Notes:
- `--noverify` skips the read-back verification, which speeds things up and avoids errors on monitors that switch away before ddcutil can verify.
- `notify-send` provides a desktop notification if config is missing; the `|| true` ensures it doesn't fail on headless/no-notification setups.

- [ ] **Step 2: Make it executable**

Run: `chmod +x kvm-switch.sh`

- [ ] **Step 3: Verify it runs (will fail gracefully without config)**

Run: `./kvm-switch.sh`
Expected: `ERROR: Config file not found` message, exit code 1.

- [ ] **Step 4: Commit**

```bash
git add kvm-switch.sh
git commit -m "feat: add kvm-switch.sh — switches both monitors via ddcutil"
```

---

### Task 2: Create install.sh — prerequisites section

**Files:**
- Create: `install.sh`

- [ ] **Step 1: Write install.sh with prerequisite installation**
...
```bash
git add install.sh
git commit -m "feat: add install.sh — prerequisite installation section"
```

---

### Task 3: Add monitor discovery to install.sh

**Files:**
- Modify: `install.sh`

- [ ] **Step 1: Append monitor detection logic after prerequisites**

Add the following after the "Prerequisites installed." line:
...
```bash
git add install.sh
git commit -m "feat: add monitor detection to install.sh"
```

---

### Task 4: Add input source discovery and config writing to install.sh

**Files:**
- Modify: `install.sh`

- [ ] **Step 1: Append input discovery, config writing, and keybinding setup**

Add the following after the monitor detection section:
...
```bash
git add install.sh
git commit -m "feat: add input discovery, config writing, and GNOME keybinding to install.sh"
```
...
Run: `sudo ./install.sh` (Note: only the apt/modprobe/usermod steps need sudo; consider whether to restructure to minimize sudo usage, or run the whole thing with sudo.)

Actually, the script uses `sudo` internally for the commands that need it, so run as normal user:

Run: `./install.sh`

Follow the interactive prompts. Verify:
- ddcutil is installed
- i2c-dev module is loaded
- Monitors are detected (note actual bus numbers and model strings for future reference)
- Current input codes are shown
- Config file is written to `~/.config/kvm-at-home/config`
- GNOME keybinding is registered

- [ ] **Step 2: Verify config file**

Run: `cat ~/.config/kvm-at-home/config`
Expected: Bus numbers and hex input codes matching what you selected.

- [ ] **Step 3: Test the switch**

Press Ctrl+Alt+P. Both monitors should switch to the other machine's inputs.

- [ ] **Step 4: Test from the other machine**

On Computer 2, clone the repo, run `./install.sh`, and verify Ctrl+Alt+P switches back.

- [ ] **Step 5: Document any LG quirks**

If the LG monitor doesn't switch reliably, note the behavior for the "LG VCP Support" edge case in the spec. Try:
- Different VCP codes
- Adding `--sleep-multiplier 2` to the ddcutil command
- Checking if DDC/CI is enabled in the LG OSD
